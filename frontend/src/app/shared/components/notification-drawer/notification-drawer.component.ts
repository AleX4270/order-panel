    ...
    template: `
        <div class="drawer drawer-end">
            ...
            <div class="drawer-side">
                <div class="drawer-overlay" (click)="notificationDrawerService.toggle()"></div>
...
export class NotificationDrawerComponent {
    ...
    protected drawerCheckbox = viewChild<ElementRef>('drawerCheckbox');

    constructor() {
        effect(() => {
            const drawerState = this.notificationDrawerService.drawerState();
            const checkbox = this.drawerCheckbox();

            if(checkbox) {
                checkbox.nativeElement.checked = drawerState;
            }
        })
    }
}
